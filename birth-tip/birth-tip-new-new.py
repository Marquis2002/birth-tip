    ...
    while 1:
        ...
        for line in lines:
            if line.find(name) != -1:
                ...
                relation = line[find_nth_(line, 1) + 1:]
...
class Index(tk.Frame):
    def __init__(self, parent=None):
        ...
        self.frm1 = tk.Frame()
        # 当前所选择的确定的亲友的信息，包括姓名、与用户的关系、出生日期、
        # 生日计划信息,包括下次生日距离今天的天数、距离下次生日前n
        # 天的日期、预计准备制定生日计划的日期
        # text1 = f"The date of your {person.relation}, {person.name}'s next birthday is {next_birth}, \nsince {person.name} was born on {person.birthday}\n"
        ...
        self.label1 = tk.Label(self.frm1, text=text1)
        self.label2 = tk.Label(self.frm1, text=text2)
        self.label3 = tk.Label(self.frm1, text=text3)
        self.label4 = tk.Label(self.frm1, text=text4)
        self.label5 = tk.Label(self.frm1, text=text5)
        ...
        self.label2_2 = tk.Label(self.frm2, text=f"{file_contents}")
        # self.frm2.pack()
        self.label2_2.pack()
...
def final_outcome(next_birth, distance_days, aiming_date, aiming_saturday):
    ...
    #
    # ttk.Button(frm, text="Show All Plans", command=show_all()).grid(column=0, row=5)
    # # Still two important problems:
    # # 1. How to change the frame/window
    # # 2. How to deal with the files
    # root.mainloop()
